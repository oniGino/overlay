...
SLOT="6"
KEYWORDS="~amd64"
IUSE="crypt djvu epub image-backend markdown mobi +pdf phonon +postscript qml share speech +tiff X"
...
DEPEND="
	...
	image-backend? ( >=kde-apps/libkexiv2-${PVCUT}:6 )
...
PATCHES=(
	...
	"${FILESDIR}/${P}-x11-optional.patch" # make x11 libraries optional
)
src_configure() {
	local mycmakeargs=(
		-DHAVE_X11=$(usex X TRUE FALSE)
		...
		$(cmake_use_find_package image-backend KExiv2Qt6)
