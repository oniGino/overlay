...
PLOCALES="be bg bn ca cs da de el en_GB es et eu fa fi fr gl he hr hu id it ja kk km
lg lt nl pl pt pt_BR ro ru si_LK sk sl sr sr@latin sv te tr ug uk vi zh_CN zh_TW"

inherit l10n fdo-mime gnome2-utils flag-o-matic git-r3 autotools-utils
...
RDEPEND="adplug? ( media-libs/adplug )
	...
	gtk? ( >=x11-libs/gtk+-2.12:2 x11-libs/gtkglext dev-libs/jansson )
...
src_prepare() {
	if use nls; then
		for lang in $(l10n_get_locales disabled); do
			rm "${S}/po/${lang}.po" || die
			sed -i '/'${lang}'/d' "${S}/po/LINGUAS" || die
		done
	fi
	./autogen.sh
}
