...
src_prepare() {
	...
	for x in Faenza Faenza-Dark Faenza-Ambiance Faenza-Darker Faenza-Darkest Faenza-Radiance; do
		for res in 16 22 24 32 48 64 96; do
			for cat in apps actions categories emblems devices extras mimetypes places status stock; do
				if [ -d "${x}"/${cat}/${res} ]; then
					mkdir -p "${x}"/${res}x${res} || die
					ln -s  ../${cat}/${res} \
						"${x}"/${res}x${res}/${cat} || die
				fi
			done
		...
		for cat in apps actions categories emblems devices extras mimetypes places status stock; do
			if [ -d "${x}"/${cat}/scalable ]; then
				mkdir -p "${x}"/scalable || die
				ln -s  ../${cat}/scalable \
					"${x}"/scalable/${cat} || die
			fi
		done
