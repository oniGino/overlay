...
SRC_URI="
	...
	https://github.com/xbmc/libdvdread/archive/${LIBDVDREAD_VERSION}.tar.gz
		-> libdvdread-${LIBDVDREAD_VERSION}.tar.gz
	https://mirrors.kodi.tv/build-deps/sources/apache-groovy-binary-${APACHE_GROOVY_VERSION}.zip
		-> apache-groovy-binary-${APACHE_GROOVY_VERSION}.zip
	https://mirrors.kodi.tv/build-deps/sources/commons-lang3-${APACHE_COMMONS_LANG_VERSION}-bin.tar.gz
		-> commons-lang3-${APACHE_COMMONS_LANG_VERSION}-bin.tar.gz
	https://mirrors.kodi.tv/build-deps/sources/commons-text-${APACHE_COMMONS_TEXT_VERSION}-bin.tar.gz
...
IUSE="airplay alsa bluetooth bluray caps cec +css dbus doc eventclients gbm gles lcms libusb lirc mariadb mysql nfs +optical pipewire pulseaudio raspberry-pi samba test udf udev upnp vaapi vdpau wayland webserver X +xslt zeroconf ${CPU_FLAGS}"
REQUIRED_USE="
	${PYTHON_REQUIRED_USE}
	^^ ( gbm wayland X )
	?? ( mariadb mysql )
...
COMMON_TARGET_DEPEND="${PYTHON_DEPS}
	...
	>=media-video/ffmpeg-${FFMPEG_VERSION}:=[openssl,encode,postproc,vaapi?,vdpau?,X?]
	udf? (
...
src_unpack() {
	...
	unpack apache-groovy-binary-${APACHE_GROOVY_VERSION}.zip
...
src_configure() {
	local mycmakeargs=(
		...
		-Dgroovy_SOURCE_DIR="${WORKDIR}/groovy-${APACHE_GROOVY_VERSION}"
