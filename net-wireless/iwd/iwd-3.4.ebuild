...
src_configure() {
	...
	local myeconfargs=(
		...
		"$(use_enable ofono)"
		"$(use_enable wired)"
		"$(use_enable systemd systemd-service)"
		"$(use_with systemd systemd-unitdir=$(systemd_get_systemunitdir))"
		"$(use_with systemd systemd-modloaddir=${EPREFIX}/usr/lib/modules-load.d)"
		"$(use_with systemd systemd-networkdir=$(systemd_get_utildir)/network)"
	)
